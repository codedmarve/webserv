...
int Response::GET() {
    ...
    if (req_config_.getAutoindex() && file_.is_directory()) {
...
void Response::createResponse() {
  ...
  if (headers_.count("Status")) {
    ...
    status_code_str = to_string(status_code_) + " " + g_status[status_code_];
  }

  // Construct response line
  response_ = req_config_.getProtocol() + " " + status_code_str + "\r\n";
...
bool Response::checkAuth() {
  std::string authorizationHeader = req_config_.getHeader("Authorization");
  ...
  return (token == req_config_.getAuth());
}


bool Response::isCGI(std::string extension) {
  const std::map<std::string, std::string>& cgi = req_config_.getCGI();
