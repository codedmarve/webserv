...
/*   Updated: 2024/03/24 21:27:51 by alappas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Servers::handleIncomingConnection(int server_fd){
	...
	// int flags = fcntl(new_socket, F_SETFL, O_NONBLOCK, FD_CLOEXEC);
	// if (flags == -1)
	// {
	// 	std::cerr << "Fcntl failed" << std::endl;
	// 	close(new_socket);
	...
	while (finish != true){
		std::cout << "FINISH SITUATION: " << finish << std::endl;
		finish = getRequest(new_socket, request);
	// getRequest(new_socket, request);
